function loadSpikesNexus(obj, trialIterator, reloadFlag)
% loadSpikes - load spike data from phy-sorted NeuroNexus files
...
%% Grab session length data
cd(obj.dataPath{trialIterator});
if reloadFlag
    offset = obj.trialMetaData(trialIterator).offSet;
else
    offset = scanpix.nexusUtils.loadSyncNexus(trialIterator);
    obj.trialMetaData(trialIterator).offset = offset;
...
if reloadFlag || exist(fullfile(obj.dataPath{trialIterator},'spike_times.npy'),'file') ~= 2
    % for a reload we always request user interaction
...
% Align spike times to trial time
spikeTimes         = double(spikeTimes) ./ obj.params('APFs') - offset; % spike times in seconds. SAMPLING RATE HARD CODED AT 48kHz FOR NOW
...
if loadFromPhy
    ...
    clu_Depth      = clu_info.depth(goodLabel);  % I think this and the following seem to not give correct results in some cases?? Phy Issue??
...
% index for spike times within [session time - offset (i.e. time 0), trialduration]
...
% May need fixing? TBC, ask Laurenz what the point of this is
...
if reloadFlag || trialIterator == 1
    ...
    % Requires that no channels have been dropped during the sorting & curating process
    % MAY NEED FIXING - original code below
    clu_Ch_mapped = clu_Ch;
